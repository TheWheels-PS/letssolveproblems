...
int main(void) {
  ...
  임의의 한 블록의 위치를 기준점으로 삼아 왼쪽에서의 가장 높은 블럭의 개수,
  오른쪽에서 가장 높은 블럭의 개수를 left, right에 저장

  둘 중에 작은 것에 기준점 블럭의 개수를 뺀 값이 해당 위치에 고이는 빗물의
  양이다.
  */
  ...
  for (int i = 1; i < width - 1; i++) {
    ...
    // 둘 중에 작은 값에서 기준점이 되는 위치의 블록의 개수를 뺀값을 모두 더한
    // 값이 답
